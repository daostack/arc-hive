/**
    Copied from: https://github.com/daostack/arc/blob/master/test/helpers.js
...
export function getValueFromLogs(tx, arg, eventName, index = 0) {
  ...
  if (!result) {
    let msg = `getValueFromLogs: This log does not seem to have a field "${arg}": ${
      tx.logs[index].args
    }`;
    throw new Error(msg);
  }
...
export const setupAbsoluteVote = async function(
  ...
  votingMachine.params = await votingMachine.absoluteVote.getParametersHash(
    precReq,
    voteOnBehalf
  );
